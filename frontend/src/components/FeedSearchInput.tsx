...
import { activeTickerPrefix, replaceActiveTicker } from "@/lib/feedFilters";
...
const inputClass =
  "min-w-0 flex-1 border border-zinc-700 bg-zinc-950 px-2 py-1 font-mono text-[10px] text-zinc-300 placeholder:text-zinc-600 focus:border-zinc-500 focus:outline-none";
  ...
  return (
    <div ref={rootRef} className={`relative min-w-0 flex-1 ${className}`}>
      ...
      {open && (
        <ul
          role="listbox"
          className="absolute left-0 right-0 top-[calc(100%+4px)] z-40 max-h-56 overflow-y-auto border border-zinc-700 bg-zinc-950 py-1 shadow-lg shadow-black/40"
        >
                ...
                >
                  <span
                    className={`font-mono text-[10px] ${
                      selected ? "font-semibold text-zinc-300" : "text-zinc-200"
                    }`}
                  >
                    ${item.symbol}
                  </span>
